...
docs = OpenAPIHandler(ui_path = '/docs/test',info=Info(title="nmx API", version="0.2.0",contact=Contact('迷与谜&bymoye','https://nmxc.ltd','s3moye@gmail.com')))
...
ip_API_docs = EndpointDocs(
    ...
    parameters={"ip":ParameterInfo(example="8.8.8.8",description='此处输入ip地址(ipv4 | ipv6)')},
    responses={
        200: ResponseInfo(
            "获取成功",
            content=[
...
randimg_API_docs = EndpointDocs(
    ...
    parameters={"encode":ParameterInfo(description='`json`或`留空`'),
...
yiyan_API_docs = EndpointDocs(
    ...
    parameters={"c":ParameterInfo(example='a',description='''### 可选参数,可多选
|参数|说明|
|----|----|
| a  | 动画 |
| b  | 漫画 |
